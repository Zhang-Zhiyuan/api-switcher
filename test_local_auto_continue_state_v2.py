...
def _wait_for_windows_process_exit(pid: int, timeout: float = 3.0) -> bool:
...
def _api_switcher_process_output_files() -> set[Path]:
...
def test_local_hook_input_read_is_bounded_when_stdin_remains_open(tmp_path):
    script = _local_hook(tmp_path, _incomplete_settings(), "codex")
    generated = script.read_text(encoding="utf-8-sig")
    input_helper = generated.split("function Read-HookInput", 1)[1].split(
    ...
    try:
        ...
        process.stdin.write(
            json.dumps(
                {
                    "hook_event_name": "Stop",
                    "session_id": "stdin-remains-open",
                    "last_assistant_message": "I still need to finish verification.",
                }
            )
        )
        process.stdin.flush()
        returncode = process.wait(timeout=7)
    finally:
    ...
    assert "ReadToEndAsync()" in input_helper
    assert ".Wait($TimeoutMilliseconds)" in input_helper
    assert "[Console]::In.ReadToEnd()" not in input_helper
    assert returncode == 0, stderr
    assert stdout.strip() == ""
    assert elapsed < 6
    assert "Hook stdin did not close within 1000 ms" in stderr
...
def test_local_git_auto_push_has_hard_five_second_timeout(tmp_path):
    ...
    generated = script.read_text(encoding="utf-8-sig")
    timeout_helper = generated.split("function Stop-GitProcessTree", 1)[1].split(
    ...
    assert '-FilePath "taskkill.exe"' in timeout_helper
    assert "$taskkillProcess.WaitForExit(2000)" in timeout_helper
    assert "if (-not $taskkillHasExited)" in timeout_helper
    assert "$taskkillProcess.Kill()" in timeout_helper
    assert "& taskkill.exe" not in timeout_helper
    assert (
        'Stop-GitProcessTree -Process $process -Invocation $invocation '
        '-Reason "exception/finalization"'
    ) in timeout_helper
...
@pytest.mark.parametrize(
    ("hook_event_name", "payload_fields"),
    [
        (
            "Stop",
            {"last_assistant_message": "I still need to finish verification."},
        ),
        ("UserPromptSubmit", {"prompt": "start new work"}),
        ("SessionStart", {"source": "startup"}),
    ],
)
def test_local_entire_git_snapshot_has_five_second_budget(
    tmp_path,
    hook_event_name,
    payload_fields,
):
    temp_outputs_before = _api_switcher_process_output_files()
    ...
    (fake_bin / "git.cmd").write_text(
        "@echo off\n"
        "chcp 65001 >nul\n"
        "if not \"%GIT_TERMINAL_PROMPT%\"==\"0\" exit /b 91\n"
        "if /I not \"%GCM_INTERACTIVE%\"==\"Never\" exit /b 92\n"
        "if /I \"%~1\"==\"rev-parse\" (\n"
        "  echo .git\n"
        "  exit /b 0\n"
        ")\n"
    ...
    env["GCM_INTERACTIVE"] = "caller-value"
    session_id = f"slow-git-status-{hook_event_name.lower()}"
    payload = {
    ...
    try:
        ...
        if nested_pid is None and nested_pid_path.exists():
            nested_pid = int(nested_pid_path.read_text(encoding="ascii").strip())
        if nested_pid is not None and _windows_process_exists(nested_pid):
            subprocess.run(
    ...
    if hook_event_name == "Stop":
        assert json.loads(result.stdout)["decision"] == "block"
        assert _read_state(config_dir)[_state_key("codex", session_id)]["count"] == 1
    else:
        assert result.stdout.strip() == ""
    ...
    assert nested_process_exited, f"nested child PID {nested_pid} survived Git timeout cleanup"
    assert _api_switcher_process_output_files() == temp_outputs_before
    generated = script.read_text(encoding="utf-8-sig")
    timeout_helper = generated.split("function Stop-GitProcessTree", 1)[1].split(
    ...
    assert "$Invocation.JobHandle.Dispose()" in timeout_helper
    assert "using taskkill fallback" in timeout_helper
    assert 'GIT_TERMINAL_PROMPT = "0"' in timeout_helper
    assert 'GCM_INTERACTIVE = "Never"' in timeout_helper
    assert "SetEnvironmentVariable(" not in timeout_helper
    assert "api-switcher-git-command-" not in timeout_helper
    assert "api-switcher-taskkill-" not in timeout_helper
