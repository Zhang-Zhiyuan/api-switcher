...
def test_profile_lock_guard_returns_false_when_lock_files_are_free(tmp_path, monkeypatch):
    ...
    assert manager.is_profile_lock_held(_profile(profile_dir)) is False
    assert checked == [lock_file]


def test_profile_lock_guard_returns_true_for_held_lock(tmp_path, monkeypatch):
    profile_dir = tmp_path / "profile"
    profile_dir.mkdir()
    lock_file = profile_dir / "SingletonCookie"
    lock_file.touch()
    manager = BrowserDataManager()
    monkeypatch.setattr(manager, "_is_file_locked", lambda _path: True)

    assert manager.is_profile_lock_held(_profile(profile_dir)) is True


def test_profile_lock_guard_fails_closed_when_lock_check_raises(tmp_path, monkeypatch):
    profile_dir = tmp_path / "profile"
    profile_dir.mkdir()
...
def test_profile_lock_guard_fails_closed_for_missing_profile(tmp_path):
    manager = BrowserDataManager()

    assert manager.is_profile_lock_held(_profile(tmp_path / "missing")) is True
