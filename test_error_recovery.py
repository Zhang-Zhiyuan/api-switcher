...
def _assert_bounded_git_snapshot_script(script: str) -> None:
    assert "Invoke-GitCommandWithTimeout" in script
    assert "Invoke-GitCommandWithinBudget" in script
    assert "Stop-GitProcessTree" in script
    ...
    assert '-Arguments @("add", "-A")' in script
    assert '@("config", "user.email", "auto@api-switcher.local")' in script
    assert '"commit", "--no-verify", "--no-gpg-sign"' in script
    assert '"-c", "commit.gpgSign=false"' in script
    assert "git add -A 2>&1" not in script
    assert "git commit --no-verify" not in script
...
def test_script_generation():
    ...
    _assert_bounded_git_snapshot_script(claude_script)
    ...
    _assert_bounded_git_snapshot_script(codex_script)
...
def test_local_codex_hooks_preserve_existing_entries(tmp_path, monkeypatch):
    ...
    provider.save_settings(auto_settings)
    provider.install_hook_script(settings=auto_settings)
    provider.register_hook(settings=auto_settings)
...
def test_local_claude_hook_repair_backs_up_invalid_settings_json(tmp_path, monkeypatch):
    ...
    stop_commands = [
        hook["command"]
        for group in settings["hooks"].get("Stop", [])
        for hook in group["hooks"]
    ]
    assert not any("auto_continue_stop.ps1" in command for command in stop_commands)
...
def test_local_status_requires_prompt_snapshot_hooks_when_git_snapshot_enabled(tmp_path, monkeypatch):
    ...
    codex.install_hook_script(settings=auto_settings)
    ...
    claude.install_hook_script(settings=auto_settings)
...
def test_local_claude_permission_only_status_does_not_require_stop(tmp_path, monkeypatch):
    ...
    claude_settings["hooks"].pop("PermissionRequest")
...
def test_local_claude_status_reports_permission_mode_drift(tmp_path, monkeypatch):
    ...
    claude_settings["skipDangerousModePermissionPrompt"] = True
    provider.get_claude_settings_path().write_text(
        json.dumps(claude_settings),
        encoding="utf-8",
    )
