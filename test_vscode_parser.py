...
def test_read_vscode_settings_rejects_non_object_json(monkeypatch, tmp_path):
...
def test_clear_claude_profile_overrides_resets_model_and_dangerous_permissions():
    settings = vscode_parser.clear_claude_profile_overrides(
        {
            "claudeCode.selectedModel": "deepseek-v4",
    ...
    cleared = vscode_parser.clear_claude_profile_overrides(settings)
    assert "claudeCode.selectedModel" not in cleared
