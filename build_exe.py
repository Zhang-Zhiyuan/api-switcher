...
DIRECTORY_CLEANUP_RETRY_ATTEMPTS = 20
DIRECTORY_CLEANUP_RETRY_DELAY_SECONDS = 0.2
...
def check_pyinstaller() -> bool:
    try:
        ...
        try:
            install_target = (
                ["-r", str(RELEASE_REQUIREMENTS_PATH)]
                if RELEASE_REQUIREMENTS_PATH.is_file()
                else ["pyinstaller==6.21.0"]
            )
...
def create_spec_file(bundle_mode: str = DEFAULT_BUNDLE_MODE) -> None:
    ...
    disable_windowed_traceback=False,
        ...
        version={str(VERSION_INFO_PATH)!r},
        {icon_line}
)
"""
    else:
...
def _rmtree_with_retry(path: Path) -> None:
    last_error = None
    for attempt in range(DIRECTORY_CLEANUP_RETRY_ATTEMPTS):
        try:
            ...
            last_error = exc
            if attempt + 1 >= DIRECTORY_CLEANUP_RETRY_ATTEMPTS:
                break
            time.sleep(DIRECTORY_CLEANUP_RETRY_DELAY_SECONDS)
...
def _isolated_smoke_test_env(root: Path) -> dict[str, str]:
    ...
    isolated_paths = {
        ...
        "API_SWITCHER_DATA_DIR": root / "api-switcher-data",
        "HOME": root / "home",
    ...
    # USERPROFILE takes precedence on Windows. Remove the legacy fallback so a
    # child cannot reconstruct the real profile path from inherited variables.
...
@contextmanager
def _isolated_smoke_directory():
    root = Path(tempfile.mkdtemp(prefix="api-switcher-smoke-"))
    try:
        yield root
    finally:
...
def smoke_test_exe(exe_path: Path, timeout_seconds: float = 8.0) -> bool:
    ...
    try:
        with _isolated_smoke_directory() as temp_dir:
            isolated_env = _isolated_smoke_test_env(temp_dir)
            try:
            ...
            try:
                ...
                if not _terminate_process_tree(proc):
                    print("Smoke test failed: packaged app could not be terminated.", flush=True)
                    return False
                print("Smoke test passed: packaged app stayed running.", flush=True)
                return True
            except Exception as exc:
                _terminate_process_tree(proc)
                print(f"Smoke test failed while waiting for packaged app: {exc}", flush=True)
                return False
        ...
        print(f"Smoke test failed: could not create or clean up isolated state: {exc}", flush=True)
        return False


def _wait_for_process_exit(proc: subprocess.Popen, timeout_seconds: float) -> bool:
    try:
...
def _terminate_process_tree(proc: subprocess.Popen) -> bool:
    ...
    if _is_windows():
        try:
            ...
            print("Smoke-test taskkill timed out; terminating the launcher directly.", flush=True)
        except OSError as exc:
            print(f"Smoke-test taskkill failed; terminating the launcher directly: {exc}", flush=True)

        if _wait_for_process_exit(proc, PROCESS_EXIT_TIMEOUT_SECONDS):
            return True
    ...
    try:
        ...
        pass
    return _wait_for_process_exit(proc, PROCESS_EXIT_TIMEOUT_SECONDS)
