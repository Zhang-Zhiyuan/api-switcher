...
def test_common_tab_overview_masks_sensitive_values(monkeypatch):
    ...
    # Keep this test independent from a developer machine's Codex .env or
    # persistent user environment; it specifically exercises auth.json
    # fallback masking.
    monkeypatch.setattr("ui.tabs.common_tab._codex_env_value", lambda _key: "")
    ...
    assert "sha256:d65c544d…3456" in text
    assert "OPENAI_API_KEY=sha256:36825995…cdef" in text
...
def test_common_tab_overview_uses_codex_provider_env_key(monkeypatch):
    ...
    assert "DEEPSEEK_API_KEY=sha256:04e2d578…cdef" in text
...
def test_common_tab_overview_marks_openai_auth_provider(monkeypatch):
    ...
    assert "OPENAI_API_KEY=sha256:a290d1b2…cdef" in text
