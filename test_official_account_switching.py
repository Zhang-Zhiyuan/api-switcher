...
def test_switch_claude_account_clears_api_overrides_and_resets_third_party_model(isolated_accounts):
    ...
    vscode_parser.write_vscode_settings({
        "claudeCode.selectedModel": "GLM-5.1",
...
def test_json_readers_reject_non_object_json(isolated_accounts):
    ...
    with pytest.raises(ValueError):
        parser.read_claude_settings()
    with pytest.raises(ValueError):
        parser.read_claude_config()
    with pytest.raises(ValueError):
        parser.read_claude_credentials()
    with pytest.raises(ValueError):
...
def test_apply_codex_official_account_preserves_existing_official_model():
    ...
    assert updated["model"] == "gpt-user-choice"
...
def test_invalid_account_snapshot_is_reported(isolated_accounts):
    ...
    broken = CodexAccountProfile(
        name="broken",
        auth_json_ref="codex-account:broken:auth_json",
    )
...
def test_account_matching_accepts_legacy_display_identity(isolated_accounts):
    ...
    legacy_ref = "claude-account:Legacy Display:credentials"
    security.set_secret_json(legacy_ref, credentials)
    profile_manager.save_claude_account_profile(
        ClaudeAccountProfile(name="Legacy Display", credentials_ref=legacy_ref, identity="Same Display")
    )
...
def test_account_matching_rejects_same_display_with_different_stable_identity(isolated_accounts):
    ...
    legacy_ref = "claude-account:Legacy Display:credentials"
    security.set_secret_json(legacy_ref, legacy_credentials)
    profile_manager.save_claude_account_profile(
        ClaudeAccountProfile(name="Legacy Display", credentials_ref=legacy_ref, identity="Same Display")
    )
