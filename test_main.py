...
def test_reconcile_auto_continue_installations_delegates_to_manager():
    expected = {"codex": True, "claude": True}
    manager = SimpleNamespace(reconcile_all_installations=lambda: expected)

    assert main.reconcile_auto_continue_installations(manager) == expected


def test_reconcile_auto_continue_installations_does_not_break_startup_on_error(caplog):
    def fail():
        raise RuntimeError("broken hook config")

    manager = SimpleNamespace(reconcile_all_installations=fail)

    assert main.reconcile_auto_continue_installations(manager) == {}
    assert "Failed to reconcile local auto-continue hooks" in caplog.text
...
def test_shutdown_clears_pending_ui_callbacks(monkeypatch):
    ...
    probe_cleanup = []
    local_proxy_module.cleanup_isolated_mihomo_sessions_for_shutdown = lambda: probe_cleanup.append(True)
