...
def _short_secret(value: object, prefix: int = 8, suffix: int = 4) -> str:
    ...
    if not text:
        return ""
    # Never reveal a short credential in full. A stable fingerprint plus the
    # final characters is enough to distinguish profiles without exposing it.
    fingerprint = hashlib.sha256(text.encode("utf-8", errors="replace")).hexdigest()[:8]
    tail = text[-max(1, int(suffix)):] if len(text) >= max(1, int(suffix)) else ""
    return f"sha256:{fingerprint}" + (f"…{tail}" if tail else "")
...
def _build_overview_text(claude: dict, codex_cfg: dict, codex_auth: dict, vscode: dict) -> str:
    ...
    auth_label = "Auth Token" if env.get("ANTHROPIC_AUTH_TOKEN") else "API Key"
    lines.append(f"  {auth_label}:  {_short_secret(token) if token else '(未设置)'}")
