...
remote_auto_continue = LazyModule("core.remote_auto_continue")
remote_git_login = LazyModule("core.remote_git_login")
...
class SSHTab(ctk.CTkScrollableFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self._proxy_subscription_profile_new_mode_selected = False
        ...
        self._proxy_manual_hot_update_button = None
        ...
        self._proxy_strict_privacy_var = ctk.BooleanVar(value=False)
        self._proxy_strict_privacy_check = None
    ...
    def _build_deployment_sections(self):
        ...
        for entry in (self._proxy_subscription_name_entry, self._proxy_subscription_entry):
            ...
            entry.bind("<<Paste>>", self._on_proxy_subscription_profile_clipboard_edit, add="+")
            entry.bind("<<Cut>>", self._on_proxy_subscription_profile_clipboard_edit, add="+")
            entry.bind("<FocusOut>", self._update_proxy_subscription_profile_form_controls, add="+")
        ...
        self._proxy_fetch_button.grid(row=0, column=0, sticky="ew")
        self._proxy_manual_hot_update_button = ctk.CTkButton(
            proxy_sub_action_frame,
            text="刷新并热更新",
            width=104,
            command=self._run_proxy_manual_hot_update,
            **button_style("accent", compact=True),
        )
        self._proxy_manual_hot_update_button.grid(row=0, column=1, sticky="ew", padx=(8, 0))
        ...
        self._proxy_auto_refresh_check.grid(row=1, column=0, sticky="w", pady=(6, 0))
        ...
        self._proxy_periodic_update_check.grid(row=1, column=1, sticky="w", padx=(8, 0), pady=(6, 0))
        ...
        ctk.CTkLabel(
            ...
            text_color=COLORS["muted"],
            font=font(12),
        ).grid(row=1, column=3, sticky="w", padx=(4, 0), pady=(6, 0))
        ...
        self._proxy_quality_cancel_button = ctk.CTkButton(
            proxy_node_actions,
            text="取消检测",
            width=112,
            command=self._cancel_proxy_subscription_quality,
            state="disabled",
            **button_style("secondary", compact=True),
        )
        ...
        self._proxy_hot_update_button.pack(anchor="e", pady=(6, 0))
        ctk.CTkLabel(
            proxy_node_actions,
            text="设置",
            text_color=COLORS["muted"],
        ...
        self._proxy_strict_privacy_check = ctk.CTkCheckBox(
            ...
            variable=self._proxy_strict_privacy_var,
            command=self._on_proxy_strict_privacy_toggle,
            text_color=COLORS["warning"],
            font=font(12, "bold"),
        )
        ...
        self._proxy_privacy_notice_label = ctk.CTkLabel(
            proxy_controls,
            text=(
                "非 VPN/TUN：仅保护已进入 mihomo 的流量，不能拦截忽略代理的程序、"
                "系统 DNS/IPv6 或 WebRTC/UDP。应用模式后请重启远端 Codex、Claude Code、"
                "VS Code Server，并打开新 shell。"
            ),
        ...
        bind_wraplength(proxy_controls, self._proxy_privacy_notice_label, padding=20)
    ...
    def _build_proxy_subscription_picker(self):
        ...
        self._proxy_subscription_picker = ProxyNodePicker(
            ...
            on_group_quality=self._measure_proxy_subscription_qualities,
        )
    ...
    def _resume_background_work(self):
        ...
        self._sync_shared_proxy_subscription_profile()

    def _current_proxy_subscription_profile_id(self) -> str:
        ...
        try:
            ...
            return ""
        options = getattr(self, "_proxy_subscription_profile_options", {}) or {}
        return str(options.get(label) or "")

    def _sync_shared_proxy_subscription_profile(self) -> bool:
        """Reload the profile another proxy tab activated without fetching it."""

        if (
            getattr(self, "_proxy_busy", False)
        ...
        if active_id == self._current_proxy_subscription_profile_id():
            return False
        ...
        if str(state.get("saved_path") or "").strip():
            self._load_proxy_subscription_cache_for_state(state, generation)
        else:
            self._set_proxy_cache_status("本机缓存: 当前分组尚无缓存", "warning")
            self._set_proxy_status("已同步其他代理页选择的订阅分组；当前分组尚无本机缓存。", "warning")
        return True
    ...
    def _create_server(self):
        def on_save(profile, _, secret_updates):
            profile_manager.save_ssh_profile_with_secrets(profile, secret_updates)
    ...
    def _edit_server(self, name):
        ...
        def on_save(new_profile, old_profile, secret_updates):
    ...
    def _set_proxy_busy(self, busy: bool):
        ...
        if self._proxy_periodic_update_check:
            try:
                ...
                pass
        if getattr(self, "_proxy_strict_privacy_check", None):
            try:
                self._proxy_strict_privacy_check.configure(state=state)
            except Exception:
                pass
        if self._proxy_subscription_picker:
            try:
    ...
    def _refresh_proxy_subscription_profile_options(
        self,
            ...
            try:
                ...
                self._proxy_subscription_profile_combo.set(
                    (
                        ...
                        else NEW_PROXY_SUBSCRIPTION_PROFILE_LABEL
                    )
                    if preserve_editor
                    else active_label
                )
        ...
        if not isinstance(snapshot, tuple) or len(snapshot) != 3:
            ...
            try:
                is_unloaded_new_form = bool(
                    combo
                    and combo.get() == NEW_PROXY_SUBSCRIPTION_PROFILE_LABEL
                )
            except Exception:
            ...
            return bool(name or url) and (
                ...
                or bool(
                    getattr(
                        self,
                        "_proxy_subscription_profile_new_mode_selected",
                        False,
                    )
                )
            )
    ...
    def _on_proxy_subscription_profile_form_edited(self, _event=None):
        if not str(
            ...
            self._proxy_subscription_profile_new_mode_selected = True
        self._update_proxy_subscription_profile_form_controls()

    def _on_proxy_subscription_profile_clipboard_edit(self, _event=None):
        if not str(
            getattr(self, "_proxy_subscription_profile_edit_id", "") or ""
        ):
            self._proxy_subscription_profile_new_mode_selected = True
        try:
            self.after_idle(self._update_proxy_subscription_profile_form_controls)
        except Exception:
            self._update_proxy_subscription_profile_form_controls()
    ...
    def _delete_proxy_subscription_profile(self):
        ...
        profile_id = self._proxy_subscription_profile_options.get(label)
        if not profile_id:
        ...
        def do_delete():
            ...
            if state.get("saved_path"):
                self._load_proxy_subscription_cache_for_state(state, self._proxy_saved_subscription_load_generation)
    ...
    def _select_proxy_subscription_profile(self, label: str, profile_id: str):
        ...
        try:
            if not remote_proxy.try_acquire_proxy_subscription_hot_update():
                raise RuntimeError("另一个标签页正在热更新订阅，请稍后再切换分组")
            lock_owned = True
    ...
    def _load_saved_proxy_subscription_ui(self):
        ...
        try:
            ...
            self._proxy_saved_subscription_loaded = False
            self._deferred_proxy_saved_subscription_pending = True
            self._set_proxy_cache_status("本机缓存: 订阅状态读取失败", "error")
            self._set_proxy_status(
                f"读取 SSH 代理订阅状态失败: {exc}",
                "error",
            )
            return
        preserve_editor = self._proxy_subscription_profile_blocks_automatic_refresh()
        if preserve_editor:
            self._refresh_proxy_subscription_profile_options(
                state,
        ...
        self._load_proxy_subscription_cache_for_state(state, generation, auto_refresh=auto_refresh, schedule_periodic=True)

    def _load_proxy_subscription_cache_for_state(
        self,
        state: dict,
        generation: int,
        *,
    ...
    def _proxy_strict_privacy_setting(self) -> bool | None:
        """Return None until the user explicitly chooses, preserving each managed server."""

        variable = getattr(self, "_proxy_strict_privacy_var", None)
        if variable is None:
            return None
        explicit = getattr(self, "_proxy_strict_privacy_explicit", None)
    ...
    def _on_proxy_strict_privacy_toggle(self):
        enabled = bool(self._proxy_strict_privacy_var.get())
        # Do not visually commit a routing-mode change before confirmation.
        self._proxy_strict_privacy_var.set(not enabled)
        if enabled:
            title = "开启 SSH 应用层严格隐私"
            message = (
                "开启后，凡是已进入远端 mihomo 的公网流量都不再 DIRECT，"
                "并使用代理内 DoH、关闭 mihomo IPv6。\n\n"
                "这不是 VPN/TUN，无法阻止忽略代理的程序、系统 DNS/IPv6 或 WebRTC/UDP 绕过。\n\n"
                "本次只保存模式；下次部署、手动热更新或定时热更新时应用。"
                "应用后请重启远端 Codex、Claude Code、VS Code Server，并打开新 shell。"
            )
        else:
            title = "关闭 SSH 应用层严格隐私"
            message = (
                "关闭后，远端 mihomo 恢复兼容分流；未命中 AI 规则的流量可能 DIRECT。\n\n"
                "本次只保存模式；下次部署、手动热更新或定时热更新时应用。"
                "应用后请重启远端 Codex、Claude Code、VS Code Server，并打开新 shell。"
            )
        ...
        def commit():
            try:
                remote_proxy.save_proxy_subscription_state(ssh_strict_privacy=enabled)
            except Exception as exc:
                error = f"保存 SSH 严格隐私模式失败: {exc}"
                self._set_proxy_status(error, "error")
                show_toast(self.winfo_toplevel(), error, is_error=True)
                return
            self._proxy_strict_privacy_explicit = True
            self._proxy_strict_privacy_var.set(enabled)
            mode = "严格隐私" if enabled else "兼容分流"
            status = (
                f"已保存 SSH {mode}模式；下次部署/热更新时应用。"
                "应用后请重启远端 Codex、Claude Code、VS Code Server，并打开新 shell。"
            )
            self._set_proxy_status(status, "warning" if enabled else "info")
            show_toast(self.winfo_toplevel(), status)
    ...
    def _run_proxy_manual_hot_update(self):
        self._start_proxy_subscription_hot_update(manual=True)
    ...
    def _start_proxy_subscription_hot_update(self, *, manual: bool):
        ...
        if not url:
            message = f"SSH 代理{mode}热更新跳过：尚未设置订阅链接。"
            self._set_proxy_status(message, "warning")
            if manual:
                show_toast(self.winfo_toplevel(), message, is_error=True)
            self._schedule_proxy_periodic_update()
            return
        server_names = tuple(self._selected_sync_server_names())
        if not server_names:
            message = f"SSH 代理{mode}热更新跳过：尚未选择 SSH 目标。"
            self._set_proxy_status(message, "warning")
            if manual:
                show_toast(self.winfo_toplevel(), message, is_error=True)
            self._schedule_proxy_periodic_update()
        ...
        try:
            ...
            # 保存订阅会激活对应分组；必须与后续拉取和运行态更新共用同一把
            # 跨标签锁，避免先切换全局分组、随后才发现另一页正在热更新。
            self._proxy_subscription_hot_update_lock_owned = True
            try:
            ...
            strict_privacy = self._proxy_strict_privacy_setting()

            self._cancel_proxy_periodic_update()
            self._proxy_saved_subscription_load_generation += 1
            generation = self._proxy_saved_subscription_load_generation
            selected_key_before = str(
                profile.get("selected_node_key") or self._selected_proxy_subscription_node_key()
            )
            self._proxy_periodic_update_running = True
            self._set_proxy_busy(True)
            busy_started = True
            self._set_proxy_cache_status(f"本机缓存: SSH {mode}热更新中...")
            self._set_proxy_status(
                f"正在{mode}刷新订阅，并热更新 "
                f"{self._format_server_target(list(server_names))} 上运行中的代理..."
            )
        except Exception as exc:
            self._proxy_periodic_update_running = False
            if busy_started:
            ...
            if lock_owned:
                remote_proxy.release_proxy_subscription_hot_update()
                lock_owned = False
            message = f"SSH 代理{mode}热更新启动失败: {exc}"
            self._set_proxy_cache_status(f"本机缓存: SSH {mode}热更新未启动", "warning")
            self._set_proxy_status(message, "error" if manual else "warning")
            if manual:
                show_toast(self.winfo_toplevel(), message, is_error=True)
            self._schedule_proxy_periodic_update()
            return
                ...
                try:
                    ...
                    try:
                        ...
                        if str(state.get("active_profile_id") or "") != profile_id:
                            ...
                            persist_selection = len(server_names) == 1
                            refresh_kwargs = {
                                "quality_results": quality_results,
                                "profile_id": profile_id,
                                "persist_selection": persist_selection,
                            }
                            if strict_privacy is not None:
                                refresh_kwargs["strict_privacy"] = strict_privacy
                            try:
                                payload["batch"] = self._run_server_batch(
                                    list(server_names),
                                    lambda server_name: remote_proxy.refresh_running_ai_proxy_from_subscription(
                                        server_name,
                                        result.nodes,
                                        **refresh_kwargs,
                                    ),
            ...
            def finish():
                ...
                try:
                    ...
                    if not payload["fetch_ok"]:
                        message = f"SSH 代理{mode}热更新拉取失败: {payload['fetch_error']}"
                        self._set_proxy_cache_status(
                            f"本机缓存: SSH {mode}热更新拉取失败，继续使用已有节点",
                            "warning",
                        )
                        self._set_proxy_status(message, "error" if manual else "warning")
                        if manual:
                    ...
                    self._proxy_prefer_quality_sort = bool(self._proxy_quality_results)
                    selected_key = (
                        str(profile_snapshot.get("selected_node_key") or selected_key_before)
                        if len(server_names) == 1
                    ...
                    if payload["apply_error"]:
                        ...
                        successful = [
                            item for item in results if self._proxy_hot_update_result_successful(item)
                        ]
                        incomplete = [item for item in results if item not in successful]
                        skipped = [
                            item for item in incomplete if self._proxy_hot_update_result_skipped(item)
                        ]
                        result_failures = [item for item in incomplete if item not in skipped]
                        if successful and (incomplete or failures):
                            message = (
                                f"SSH 代理{mode}热更新部分成功: "
                                + " | ".join(results)
                            )
                            if failures:
                            ...
                            message = f"SSH 代理{mode}热更新完成: " + " | ".join(results)
                            severity = "success"
                        elif result_failures or failures:
                            details = [*result_failures, *failures]
                            message = f"SSH 代理{mode}热更新失败: " + " | ".join(details)
                            severity = "warning"
                        elif skipped:
                            message = f"SSH 代理{mode}热更新未执行: " + " | ".join(skipped)
                            severity = "warning"
                        else:
                    ...
                    message = f"SSH 订阅已刷新，但界面同步失败: {exc}"
                    self._set_proxy_status(message, "warning")
                    if manual:
                        show_toast(self.winfo_toplevel(), message, is_error=True)
                finally:
        ...
        try:
            threading.Thread(
                target=run,
                name=f"ssh-proxy-{mode}-hot-update",
                daemon=True,
            ).start()
            lock_owned = False
        except Exception as exc:
    ...
    def _set_proxy_subscription_nodes(self, nodes, preserve_key: str = ""):
        ...
        if self._proxy_hot_update_button:
    ...
    def _refresh_proxy_subscription_action_hint(self):
        if not self._proxy_subscription_action_hint_label:
            return
        scope = self._proxy_subscription_batch_scope_label()
    ...
    def _fetch_proxy_subscription(self, auto: bool = False, show_message: bool = True):
        ...
        def run():
            ...
            def finish():
                ...
                if str(current_state.get("active_profile_id") or "") != profile_id:
                    self._refresh_proxy_subscription_profile_options(current_state)
                    self._apply_proxy_subscription_profile_inputs(current_state)
                    self._set_proxy_subscription_nodes(())
                    ...
                    if str(current_state.get("saved_path") or "").strip():
                        self._load_proxy_subscription_cache_for_state(current_state, generation)
                    if show_message:
                ...
                if not payload["ok"]:
                    ...
                    if cached and cached.nodes:
                        ...
                        self._proxy_latency_results = {}
                        self._proxy_latency_server_count = 0
                        self._proxy_quality_results = payload.get("qualities") or {}
                        self._proxy_prefer_quality_sort = bool(self._proxy_quality_results)
                        selected_key = str(cached_state.get("selected_node_key") or "")
                        self._set_proxy_subscription_nodes(cached.nodes, preserve_key=selected_key)
                        self._select_proxy_subscription_node_by_key(selected_key)
                    ...
                    if (cached and cached.nodes) or retain_in_memory:
                        ...
                        severity = "warning"
                        self._set_proxy_cache_status("本机缓存: 拉取失败，继续使用已有节点", "warning")
                    else:
                        self._set_proxy_subscription_nodes(())
                ...
                self._proxy_latency_results = {}
                self._proxy_latency_server_count = 0
                self._proxy_quality_results = remote_proxy.load_proxy_subscription_qualities(state)
                self._proxy_prefer_quality_sort = bool(self._proxy_quality_results)
                selected_key = str(state.get("selected_node_key") or "")
                self._set_proxy_subscription_nodes(result.nodes, preserve_key=selected_key)
    ...
    def _proxy_quality_candidate_nodes(self, nodes=None):
        return list(nodes if nodes is not None else self._proxy_subscription_batch_nodes())

    def _proxy_subscription_quality_scope(self, region: str = "", nodes=None):
        ...
        if region and self._proxy_subscription_picker:
            candidates = self._proxy_subscription_picker.group_items(region)
    ...
    def _cancel_proxy_subscription_quality(self):
        ...
        self._set_proxy_status("正在停止家宽检测；已开始的 DNS 解析/网络请求结束后即停止...", "warning")

    def _measure_proxy_subscription_qualities(self, region: str = "", nodes=None):
        if self._proxy_busy:
            show_toast(self.winfo_toplevel(), "远端代理操作正在进行中，请稍等", is_error=True)
            return
        if not self._proxy_subscription_nodes:
        ...
        candidates, scope_label = self._proxy_subscription_quality_scope(region, nodes)
        if not candidates:
        ...
        self._proxy_quality_cancel_event = cancel_event
        self._set_proxy_busy(True)
        self._set_proxy_status(
        ...
        def run():
            ...
            def finish():
                ...
                self._set_proxy_subscription_nodes(self._proxy_subscription_nodes, preserve_key=best_key)
                self._proxy_quality_cancel_event = None
                self._set_proxy_busy(False)
                tested_count = sum(
                    1
                ...
                quality = batch_results.get(best_key) or self._proxy_quality_results.get(best_key)
    ...
    def _fastest_proxy_subscription_node(self, nodes=None):
        return remote_proxy.best_proxy_subscription_node_by_latency(
            nodes if nodes is not None else self._proxy_subscription_nodes,
            self._proxy_latency_results,
            self._proxy_quality_results,
        )

    def _use_selected_proxy_subscription_node(
        self,
    ...
    def _hot_update_selected_proxy_subscription_node(self):
        if getattr(self, "_proxy_periodic_update_running", False):
            message = "SSH 代理定时热更新正在运行，请稍后再试"
            self._set_proxy_status(message, "warning")
            show_toast(self.winfo_toplevel(), message, is_error=True)
            return
        if getattr(self, "_proxy_busy", False):
            message = "AI 代理操作正在进行中，请稍后再试"
            self._set_proxy_status(message, "warning")
            show_toast(self.winfo_toplevel(), message, is_error=True)
            return
        if getattr(self, "_ssh_busy", False):
            message = "SSH 操作正在进行中，请稍后再试"
            self._set_proxy_status(message, "warning")
            show_toast(self.winfo_toplevel(), message, is_error=True)
            return
        if not getattr(self, "_proxy_subscription_picker", None):
            message = "节点列表尚未加载"
            self._set_proxy_status(message, "warning")
            show_toast(self.winfo_toplevel(), message, is_error=True)
            return
        item = self._proxy_subscription_picker.selected_item()
        if not item:
            message = "请先拉取订阅并选择要热更新的节点"
            self._set_proxy_status(message, "warning")
            show_toast(self.winfo_toplevel(), message, is_error=True)
            return
        server_names = self._require_selected_servers(status_setter=self._set_proxy_status)
        if not server_names:
            return

        proxy_text = remote_proxy.format_proxy_node(item.node)
        node_summary = remote_proxy.describe_proxy_node(item.node)
        candidate_nodes = tuple(self._proxy_subscription_nodes)
        quality_results = dict(self._proxy_quality_results)
        target_label = self._format_server_target(server_names)
        strict_privacy = self._proxy_strict_privacy_setting()
        lock_owned = False
        try:
            ...
            state = remote_proxy.load_proxy_subscription_state()
            profiles = state.get("profiles") if isinstance(state.get("profiles"), dict) else {}
            profile_combo = getattr(self, "_proxy_subscription_profile_combo", None)
            profile_options = getattr(self, "_proxy_subscription_profile_options", {}) or {}
            profile_label = profile_combo.get() if profile_combo else ""
            profile_id = ""
            if profile_combo:
                profile_id = str(
                    profile_options.get(profile_label) or state.get("active_profile_id") or ""
                )
            if profile_combo and (not profile_id or not isinstance(profiles.get(profile_id), dict)):
                raise RuntimeError("当前订阅分组不存在，请重新选择后再试")
            reload_kwargs = {
                "quality_results": quality_results,
                # 多台服务器可能各自回退到不同节点，禁止批量任务争写共享选中项。
                "persist_selection": False,
            }
            if strict_privacy is not None:
                reload_kwargs["strict_privacy"] = strict_privacy
            if profile_id:
                reload_kwargs["profile_id"] = profile_id

            def worker():
                try:
                    return self._run_server_batch(
                        server_names,
                        lambda server_name: remote_proxy.reload_ai_proxy_verified(
                            server_name,
                            proxy_text,
                            candidate_nodes,
                            **reload_kwargs,
            ...
            def on_done(payload):
                if not payload["ok"]:
                    message = f"SSH 节点热更新失败: {payload['error']}"
                    self._set_proxy_status(message, "error")
                    show_toast(self.winfo_toplevel(), message, is_error=True)
                    return
                ...
                results = list(batch.get("results") or [])
                failures = list(batch.get("failures") or [])
                successful = [result for result in results if self._proxy_hot_update_result_successful(result)]
                incomplete = [result for result in results if result not in successful]
                skipped = [result for result in incomplete if self._proxy_hot_update_result_skipped(result)]
                result_failures = [result for result in incomplete if result not in skipped]
                detail_parts = [*results, *failures]
                detail = " | ".join(detail_parts) if detail_parts else "没有可执行的 SSH 目标"
                ...
                if successful and (incomplete or failures):
                    ...
                    outcome = "失败"
                    severity = "error"
                message = f"{target_label} 热更新当前节点 {outcome}（{node_summary}）: {detail}"
                self._set_proxy_status(message, severity)
                show_toast(self.winfo_toplevel(), message, is_error=severity != "success")

            started = self._run_proxy_ssh_task(
                f"正在将 {target_label} 上运行中的代理热更新为 {node_summary}...",
                worker,
                on_done=on_done,
            )
            if started is not False:
                ...
                message = "SSH 节点热更新线程未启动"
                self._set_proxy_status(message, "error")
                show_toast(self.winfo_toplevel(), message, is_error=True)
        except Exception as exc:
            message = f"SSH 节点热更新启动失败: {exc}"
            self._set_proxy_status(message, "error")
            show_toast(self.winfo_toplevel(), message, is_error=True)
        finally:
            if lock_owned:
    ...
    @staticmethod
    def _proxy_hot_update_result_successful(message: str) -> bool:
        text = str(message or "")
        if "已恢复更新前节点" in text or "已恢复原节点" in text:
            return False
        return "验证通过" in text or "已无重启切换到" in text

    @staticmethod
    def _proxy_hot_update_result_skipped(message: str) -> bool:
        text = str(message or "")
    ...
    def _deploy_ai_proxy(self):
        ...
        target_label = self._format_server_target(server_names)
        strict_privacy = self._proxy_strict_privacy_setting()
        if strict_privacy is True:
            privacy_description = (
                "将应用应用层严格隐私：进入 mihomo 的公网流量全部走节点，"
                "代理内 DoH，mihomo IPv6 关闭。"
            )
        elif strict_privacy is False:
            privacy_description = "将应用兼容分流：仅 AI 及命中规则的目标走节点，其余 DIRECT。"
        else:
            privacy_description = (
                "尚未显式选择隐私模式：已有受管配置保留原模式，首次部署默认兼容分流。"
            )
        install_kwargs = {"quality_results": self._proxy_quality_results}
        if strict_privacy is not None:
            install_kwargs["strict_privacy"] = strict_privacy
    ...
    def _run_proxy_ssh_task(self, busy_message: str, worker, on_done=None):
        ...
        started = self._run_ssh_task(busy_message, worker, on_done=finish)
        if started is False and self._proxy_busy:
            self._set_proxy_busy(False)
        return started
    ...
    def _run_ssh_task(self, busy_message: str, worker, on_done=None, refresh: bool = False):
        ...
        try:
            ...
            if self._remote_pull_button and self._remote_pull_options:
                self._remote_pull_button.configure(state="normal")
            payload = {"ok": False, "result": None, "error": f"工作线程启动失败: {exc}"}
            try:
                if on_done:
                    on_done(payload)
                else:
                ...
                message = f"操作结果处理失败: {callback_exc}"
                self._set_sync_status(message, "error")
                show_toast(self.winfo_toplevel(), message, is_error=True)
            if refresh:
                try:
                    ...
                    message = f"刷新 SSH 页面失败: {refresh_exc}"
                    self._set_sync_status(message, "error")
                    show_toast(self.winfo_toplevel(), message, is_error=True)
            return False
        return True
