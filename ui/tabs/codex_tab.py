...
class CodexTab(ProfileTabRendering, ctk.CTkScrollableFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self._build_ui()
        self._init_card_rendering("Codex", lambda: is_active_tab(self))
    ...
    def _build_ui(self):
        ...
        for column in range(2):
            self._api_actions.grid_columnconfigure(column, weight=1, uniform="codex_api_actions")
        ...
        account_subtitle = ctk.CTkLabel(
            self._account_title,
            text="保存并迁移 Codex ChatGPT 登录（密码可选）；切换前退出 Codex / VS Code，完成后重开；多机长期使用请分别登录",
            text_color=COLORS["muted"],
            font=font(12),
            anchor="w",
            justify="left",
    ...
    def _render_refresh_payload(self, payload: dict, generation: int):
        ...
        if self._account_runtime_label:
            if active_account:
                account_text = f"官方账号本地已写入: {active_account} | {account_runtime.get('identity')} | 服务端登录未验证"
        ...
        # Finish the upper section first. Interleaving both growing sections
        # repeatedly repositions every account's native child window on Windows.
    ...
    def _switch_account(self, name):
        def perform_switch():
            try:
                ...
                show_toast(self.winfo_toplevel(), f"已写入 Codex 账号: {name}；请完全重开 Codex / VS Code，服务端登录尚未验证")
                self.refresh()
                self._refresh_shell_state()
            except Exception as e:
    ...
    def _test_profile(self, name):
        ...
        try:
            ...
            show_toast(self.winfo_toplevel(), f"读取 API Key 失败: {exc}", is_error=True)
            return
        if not api_key:
        ...
        try:
            threading.Thread(target=run_test, name=f"codex-api-test-{name}", daemon=True).start()
        except Exception as exc:
            self._finish_profile_test(name)
    ...
    def _edit_profile(self, name):
        ...
        def on_save(data, old_profile):
            uses_openai_auth = bool(data.get("custom_requires_openai_auth"))
            api_key_ref = None if uses_openai_auth else (old_profile.api_key_ref if old_profile else None)
            secret_updates = {}

            if data.get("api_key") and not uses_openai_auth:
    ...
    def _create_profile(self):
        def on_save(data, _):
            ...
            if data.get("api_key") and not data.get("custom_requires_openai_auth"):
                api_key_ref = f"codex:{data['name']}:api_key"
                secret_updates[api_key_ref] = data["api_key"]
