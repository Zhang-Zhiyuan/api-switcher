...
class ClaudeTab(ctk.CTkScrollableFrame):
    ...
    def _build_ui(self):
        ...
        subtitle_label = ctk.CTkLabel(
            title_area,
            text="API 配置管理第三方端点和密钥；官方账号支持保存登录快照与加密跨电脑迁移",
        ...
        account_subtitle = ctk.CTkLabel(
            self._account_title,
            text="保存 Claude Code 登录快照；可单独导出加密登录包到另一台电脑使用，切换后新终端生效",
            text_color=COLORS["muted"],
            font=font(12),
            anchor="w",
            justify="left",
        ...
        # Account transfers have three actions; reserve their own row even on wide screens.
    ...
    def _import_account_login(self):
        open_account_transfer(self, "claude", exporting=False)
