...
class BrowserDataManager:
    ...
    @staticmethod
    def _profile_lock_candidates(profile_dir: Path) -> tuple[Path, ...]:
        return (
    ...
    def is_profile_lock_held(self, profile: BrowserProfile) -> bool:
        """Fast, non-mutating lock check for a specific Chromium profile.

        Unlike ``is_browser_running`` this deliberately avoids WMI/PowerShell.
        It is used as a strict-privacy launch guard: an unreadable lock state is
        treated as occupied so Chromium cannot silently reuse a process whose
        startup flags were fixed by an earlier launch.
        """

        try:
            profile_dir = self._resolve_profile_dir(profile)
        except Exception as e:
            logger.warning(f"Unable to resolve browser profile for lock check: {e}")
            return True
        for candidate in self._profile_lock_candidates(profile_dir):
            try:
                if candidate.exists() and self._is_file_locked(candidate):
                    logger.debug(f"Browser profile lock held: {candidate}")
                    return True
            except Exception as e:
