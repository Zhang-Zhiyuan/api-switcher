...
class SettingsLoadStatus(str, Enum):
    """Outcome of reading a provider's persisted auto-continue settings."""
...
class SettingsLoadResult:
    """Structured settings result used where failure semantics affect writes."""

    status: SettingsLoadStatus
    settings: Optional[AutoContinueSettings] = None
...
class AutoContinueProvider(ABC):
    ...
    def install_hook_script_for_settings(self, settings: AutoContinueSettings) -> None:
        """Install the generated script using pending settings when supported."""
        import inspect

        params = inspect.signature(self.install_hook_script).parameters
        if "settings" in params:
    ...
    def load_settings_result(self) -> SettingsLoadResult:
        """Read settings while distinguishing absence, invalid data, and I/O failure.

        Callers that may mutate hook state must use this method.  The legacy
        ``load_settings`` API intentionally continues to collapse every
        non-loaded outcome to ``None``.
        """
        import json
        ...
        try:
            ...
            # The file can disappear after a directory scan or existence check.
            return SettingsLoadResult(SettingsLoadStatus.MISSING)
        except UnicodeError as exc:
            logger.warning(f"Invalid settings encoding in {settings_path}: {exc}")
            return SettingsLoadResult(SettingsLoadStatus.INVALID, error=exc)
        except OSError as exc:
            logger.error(f"Unable to read settings from {settings_path}: {exc}")
            return SettingsLoadResult(SettingsLoadStatus.IO_ERROR, error=exc)
        except Exception as exc:
            logger.error(f"Unexpected error reading settings from {settings_path}: {exc}")
            return SettingsLoadResult(SettingsLoadStatus.ERROR, error=exc)

        try:
            data = json.loads(raw)
        except json.JSONDecodeError as exc:
            logger.warning(f"Invalid settings JSON in {settings_path}: {exc}")
            return SettingsLoadResult(SettingsLoadStatus.INVALID, error=exc)
        except Exception as exc:
            logger.error(f"Unexpected error parsing settings from {settings_path}: {exc}")
            return SettingsLoadResult(SettingsLoadStatus.ERROR, error=exc)

        if not isinstance(data, dict):
            error = ValueError(
                f"settings root must be an object, got {type(data).__name__}"
            )
            logger.warning(f"Invalid settings in {settings_path}: {error}")
            return SettingsLoadResult(SettingsLoadStatus.INVALID, error=error)
        ...
        try:
            ...
            return SettingsLoadResult(SettingsLoadStatus.LOADED, settings=settings)
        except (TypeError, ValueError) as exc:
            logger.warning(f"Invalid settings in {settings_path}: {exc}")
            return SettingsLoadResult(SettingsLoadStatus.INVALID, error=exc)
        except Exception as exc:
            logger.error(f"Unexpected error validating settings in {settings_path}: {exc}")
            return SettingsLoadResult(SettingsLoadStatus.ERROR, error=exc)

    def load_settings(self) -> Optional[AutoContinueSettings]:
        """Load settings, preserving the historical ``settings or None`` API."""
        return self.load_settings_result().settings
    ...
    def enable(self, settings: Optional[AutoContinueSettings] = None) -> None:
        ...
        try:
            # Install hook script
            self.install_hook_script_for_settings(settings)
