...
function Read-HookInput {{
    ...
    try {{
        ...
        if (-not $readTask.Wait($TimeoutMilliseconds)) {{
            Write-Log "Hook stdin did not close within $TimeoutMilliseconds ms; ignoring incomplete input" "WARN"
            return ""
        }}
        ...
        # Disposing a StreamReader while ReadToEndAsync is pending may block on
        # the same unclosed pipe. The process exits immediately after the empty
        # result, so only dispose reads that have already completed.
        if ($null -ne $reader -and ($null -eq $readTask -or $readTask.IsCompleted)) {{
...
function Stop-GitProcessTree {{
    ...
    if (
        ...
        try {{
            ...
            if (Test-GitProcessHasExited -Process $Process) {{
                Write-Log "Git $Reason cleanup verified Job Object process tree PID $gitProcessId has exited" "INFO"
                return $true
            }}
            Write-Log "Job Object cleanup did not stop Git PID $gitProcessId; using taskkill fallback" "WARN"
        }} catch {{
            Write-Log "Job Object cleanup failed for Git PID $gitProcessId; using taskkill fallback: $_" "WARN"
        }}
    }}
    ...
    if ([Environment]::OSVersion.Platform -eq [PlatformID]::Win32NT) {{
        try {{
            # taskkill can itself block under a damaged WMI/process subsystem.
            # Capture through pipes: a hard watchdog exit can no longer leave
            # redirected output files behind in the system temp directory.
            $taskkillInvocation = Start-ApiSwitcherRedirectedProcess `
                -FilePath "taskkill.exe" `
                -Arguments @("/PID", [string]$gitProcessId, "/T", "/F")
            $taskkillProcess = $taskkillInvocation.Process
            if (-not $taskkillProcess.WaitForExit(2000)) {{
                $taskkillTimedOut = $true
                Write-Log "taskkill timed out after 2000 ms for Git process tree PID $gitProcessId" "WARN"
            ...
            Write-Log "taskkill could not run for Git process tree PID $gitProcessId`: $_" "WARN"
        }} finally {{
        ...
        if (-not $taskkillTimedOut -and $taskkillExitCode -eq 0) {{
            Write-Log "taskkill completed for Git process tree PID $gitProcessId (exit code 0)" "INFO"
        }} elseif (-not $taskkillTimedOut) {{
            $taskkillSummary = (($taskkillOutput -split "`r?`n") |
                Where-Object {{ -not [string]::IsNullOrWhiteSpace($_) }} |
                Select-Object -First 1)
            if ([string]::IsNullOrWhiteSpace($taskkillSummary)) {{ $taskkillSummary = "no output" }}
            Write-Log "taskkill failed for Git process tree PID $gitProcessId (exit code $taskkillExitCode): $taskkillSummary" "WARN"
        }}
    }}
    ...
    if (-not (Test-GitProcessHasExited -Process $Process)) {{
        try {{
            $Process.Kill()
            Write-Log "Used direct Kill fallback for Git process PID $gitProcessId" "WARN"
        }} catch {{
            Write-Log "Direct Kill fallback failed for Git process PID $gitProcessId`: $_" "WARN"
        }}
    }}

    try {{ $Process.WaitForExit(750) | Out-Null }} catch {{ }}
    $gitProcessExited = Test-GitProcessHasExited -Process $Process
    if ($gitProcessExited) {{
        Write-Log "Git $Reason cleanup verified process PID $gitProcessId has exited" "INFO"
    }} else {{
        Write-Log "Git $Reason cleanup could not verify process PID $gitProcessId exited" "ERROR"
    }}
    return $gitProcessExited
}}
...
function Invoke-GitCommandWithTimeout {{
    ...
    try {{
        ...
        # Per-child environment overrides avoid mutating the hook's process
        # environment while another thread may be reading it.
        ...
        if (-not $process.WaitForExit($TimeoutMilliseconds)) {{
            Write-Log "Git command timed out after $TimeoutMilliseconds ms; terminating process tree rooted at PID $($process.Id)" "WARN"
            Stop-GitProcessTree -Process $process -Invocation $invocation -Reason "timeout" | Out-Null
            return [pscustomobject]@{{ TimedOut = $true; ExitCode = -1; Stdout = ""; Stderr = ""; Output = "" }}
        }}
        ...
        if ($null -ne $process) {{
            # Covers exceptions from WaitForExit, ExitCode, and result-file
            # handling. Never dispose a live root and leave its descendants.
            if (-not (Test-GitProcessHasExited -Process $process)) {{
                Stop-GitProcessTree -Process $process -Invocation $invocation -Reason "exception/finalization" | Out-Null
            }}
