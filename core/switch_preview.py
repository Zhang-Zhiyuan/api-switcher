...
def build_codex_api_preview(name: str) -> SwitchPreview:
    ...
    if credentials_store == "file":
        try:
            ...
            auth = {}
            runtime = _codex_runtime_summary_without_auth(config, credentials_store)
            account_before = "file 凭据（当前无法读取）"
        else:
        ...
        # auth.json can remain on disk after Codex moves credentials to the OS
        # keyring.  Do not read or display that stale file as the active login.
        runtime = _codex_runtime_summary_without_auth(config, credentials_store)
        auth = {}
        account_before = f"{credentials_store or '无效'} 凭据存储（无法静态确认登录）"
        account_after = f"保持 {credentials_store or '无效'} 凭据存储；Provider 配置生效"
...
def _validate_claude_api_target(profile: ClaudeProfile) -> list[PreviewCheck]:
    ...
    try:
        canonical_base_url = validate_api_base_url(base_url)
        ...
        checks.append(PreviewCheck(
            ...
            "error",
            f"Base URL 无效: {exc}",
            "远程 API 使用 HTTPS；HTTP 仅用于 localhost/127.0.0.1/::1。",
        ))
    else:
        checks.append(PreviewCheck(category, "Base URL", "ok", canonical_base_url))
...
def _validate_codex_api_target(profile: CodexProfile) -> list[PreviewCheck]:
    ...
    category = "Codex CLI"
    try:
        approval_policy, sandbox_mode = profile.validate_runtime_options()
        checks.append(PreviewCheck(category, "审批策略", "ok", approval_policy))
        checks.append(PreviewCheck(category, "沙盒模式", "ok", sandbox_mode))
    except ValueError as exc:
        checks.append(PreviewCheck(
            category,
            "运行权限配置",
    ...
    if profile.custom_requires_openai_auth:
        try:
            credentials_store = _codex_credentials_store(toml_parser.read_codex_config())
        except (OSError, ValueError) as exc:
            checks.append(PreviewCheck(
                category,
                "OpenAI 认证",
                "error",
                f"无法确认 Codex 凭据存储方式: {exc}",
                "修复 config.toml 后重试。",
            ))
        else:
            if credentials_store == "file":
                try:
                    ...
                    checks.append(PreviewCheck(
                        ...
                        "OpenAI 认证",
                        "error",
                        f"无法读取当前 file 凭据: {exc}",
                        "修复 auth.json，或在 Codex CLI 重新登录。",
                    ))
                else:
                    if _codex_has_openai_auth(auth):
                        ...
                        checks.append(PreviewCheck(
                            ...
                            "OpenAI 认证",
                            "error",
                            "凭据存储为 file，但当前 auth.json 没有 ChatGPT/OpenAI token。",
                            "先在 Codex CLI 完成官方账号登录，或关闭 Provider 的 requires_openai_auth。",
                        ))
            elif credentials_store in {"auto", "keyring"}:
                checks.append(PreviewCheck(
                    category,
                    "OpenAI 认证",
                    "warning",
                    f"凭据存储为 {credentials_store}，不会信任可能遗留的 auth.json，因此无法静态确认登录状态。",
                    "切换前请运行 `codex login status` 确认官方登录有效。",
                ))
            else:
                checks.append(PreviewCheck(
                    category,
                    "OpenAI 认证",
                    "error",
                    f"Codex 凭据存储方式无效: {credentials_store or '空值'}。",
                    "在 config.toml 中改为 file、auto 或 keyring。",
                ))
    ...
    if not profile.custom_requires_openai_auth:
        try:
            env_key = profile.validated_env_key()
            checks.append(PreviewCheck(category, "环境变量名", "ok", str(env_key)))
        except ValueError as exc:
            checks.append(PreviewCheck(
                category,
                "环境变量名",
                "error",
                str(exc),
                "编辑配置并改用专用密钥变量，例如 MY_PROVIDER_API_KEY。",
            ))
...
def _codex_target_env_key(profile: CodexProfile) -> str:
    if profile.custom_requires_openai_auth:
        return "-"
    try:
        return profile.validated_env_key() or "-"
    except ValueError:
        return str(profile.custom_env_key or "无效")


def _codex_credentials_store(config: dict) -> str:
    raw_value = config.get("cli_auth_credentials_store", "auto") if isinstance(config, dict) else ""
    return str(raw_value or "").strip().lower()


def _codex_runtime_summary_without_auth(config: dict, credentials_store: str) -> dict:
    return {
        "profile_name": None,
        ...
        "auth_mode": f"{credentials_store or '无效'}（无法静态确认）",
        "auth_identity": "未读取 auth.json",
