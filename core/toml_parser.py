...
def read_codex_config() -> dict:
    ...
    try:
        ...
        if not isinstance(data, dict):
            raise ValueError(f"{CODEX_CONFIG} 的顶层 TOML 必须是表")
...
def apply_codex_official_account(config: dict) -> dict:
    ...
    if previous_provider != "openai":
        # A third-party model id is not a safe OpenAI default.  Removing it
        # lets Codex select its current official default; an existing OpenAI
        # model choice remains untouched.
        config.pop("model", None)
    return sanitize_codex_config(config)


def clear_codex_api_overrides(config: dict) -> dict:
    """Remove third-party overrides without changing the credential backend.

    This is the explicit "clear API info" operation, not an account restore.
    Keep ``auto``/``keyring`` preferences and avoid pinning an app-selected
    model; Codex can then use its own current official default.
    """
    config = sanitize_codex_config(config)
