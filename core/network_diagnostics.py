...
PROXYCHECK_API_VERSION = "24-June-2026"
...
class Ping0Quality:
    ...
    def quality_text(self) -> str:
        ...
        if self.has_paid_quality:
            if self.isidc is True:
                ip_type = "IDC机房IP"
            elif self.isidc is False:
                ip_type = "非IDC（类型待核验）"
            else:
                ip_type = "网络类型未知"
...
class ReputationInfo:
    ...
    def summary_text(self) -> str:
        ...
        if self.confidence_score is not None:
            parts.append(f"证据置信 {self.confidence_score}%")
...
def lookup_ping0_quality(
    ...
    *,
    ...
    for index, key in enumerate(keys):
        ...
        if not _should_rotate_api_key(paid_quality.error):
            break

    if attempts:
        quality.attempts = attempts
    if not allow_free_fallback:
        quality.source = quality.source or "ping0-api"
        quality.error = quality.error or "Ping0 指定 IP API 未返回有效结果"
        return quality
...
def _lookup_ping0_paid_quality(
    ...
    if returned_ip and _valid_ip(returned_ip) and not _same_ip_address(returned_ip, quality.ip):
...
def _lookup_ping0_free_geo(
    ...
    if not lines:
        ...
        return quality
    returned_ip = lines[0]
    if _valid_ip(returned_ip) and not _same_ip_address(returned_ip, quality.ip):
...
def lookup_netcoffee_reputation(
    ...
    if _payload_is_error(data) or (payload is not data and _payload_is_error(payload)):
        ...
        return info
    returned_ip = _first_text(payload, "ip", "query", "address")
    if returned_ip and _valid_ip(returned_ip) and not _same_ip_address(returned_ip, ip):
        ...
        try:
            ...
            if isinstance(risk_data, dict):
                ...
                if not _payload_is_error(risk_data) and not (risk_payload is not risk_data and _payload_is_error(risk_payload)):
                    ...
                    if not risk_ip or _same_ip_address(risk_ip, ip):
...
def lookup_proxycheck_reputation(
    ...
    for index, key in enumerate(keys_to_try):
        ...
        if key and not _should_rotate_api_key(info.error):
            break
...
def _lookup_proxycheck_reputation_once(
    ...
    safe_ip = urllib.parse.quote(ip, safe=":.")
    params = {"p": "0", "tag": "0", "ver": PROXYCHECK_API_VERSION}
    ...
    info.country_code = _geo_country_code(location, "country_code", "isocode") or _geo_country_code(
        record,
        "country_code",
        "isocode",
    )
    info.country = _geo_text(location, "country_name", "country") or _geo_text(
        record,
        "country_name",
        "country",
    )
    info.region = _geo_text(location, "region_name", "region") or _geo_text(
        record,
        "region_name",
        "region",
    )
    info.city = _geo_text(location, "city_name", "city") or _geo_text(record, "city_name", "city")
    risk_value = _first_value(detections, "risk", "risk_score", "score")
    if risk_value is None:
        risk_value = _first_value(record, "risk", "risk_score", "score")
    if risk_value is None:
        risk_value = data.get("risk")
    confidence_value = _first_value(detections, "confidence", "confidence_score")
    if confidence_value is None:
        confidence_value = _first_value(record, "confidence", "confidence_score")
    info.risk_score = _optional_int(risk_value)
    info.confidence_score = _optional_int(confidence_value)
    ...
    if info.confidence_score is not None:
        info.signals.append(f"ProxyCheck confidence={info.confidence_score}")
...
def lookup_ipqs_reputation(
    ...
    for index, key in enumerate(keys):
        ...
        if info.ok:
            ...
            return info
        attempts.append(f"{info.api_key_label} 失败: {info.error or '请求失败'}")
        if not _should_rotate_api_key(info.error):
            break
    info.attempts = attempts
    return info
...
def _lookup_ipqs_reputation_once(
    ...
    returned_ip = _first_text(payload, "ip", "query", "address")
    if returned_ip and _valid_ip(returned_ip) and not _same_ip_address(returned_ip, ip):
        info.error = f"IPQualityScore 返回 IP 不匹配: {returned_ip}"
        return info
    info.ok = True
    info.raw = payload
    ...
    info.country_code = _geo_country_code(payload, "country_code", "countryCode")
    info.country = _geo_text(payload, "country", "country_name")
    info.region = _geo_text(payload, "region", "state", "province")
    info.city = _geo_text(payload, "city")
...
def _lookup_vpnapi_reputation_once(
    ...
    returned_ip = _first_text(payload, "ip", "query", "address")
    if returned_ip and _valid_ip(returned_ip) and not _same_ip_address(returned_ip, ip):
        info.error = f"VPNAPI.io 返回 IP 不匹配: {returned_ip}"
        return info
    info.ok = True
    info.raw = payload
    ...
    info.country_code = _geo_country_code(location, "country_code")
    info.country = _geo_text(location, "country")
    info.region = _geo_text(location, "region", "state", "province")
    info.city = _geo_text(location, "city")
...
def classify_ip(
    ...
    if reputation_classification:
        return _apply_ping0_native_constraint(reputation_classification, ping0)
    ...
    if ping0 and ping0.has_paid_quality:
        ...
        if ping0.isidc is False:
            signals.append("Ping0 isidc=False 仅证明非 IDC，不能单独证明为住宅家宽。")
    risk = 35
    confidence = "低"
    ip_type = "未知"
    ...
    explicit_risk = _explicit_reputation_risk_floor(reputation, ping0)
    if explicit_risk is not None:
        risk = max(risk, explicit_risk)
        signals = _reputation_signals([item for item in reputation if item.ok]) + signals
        signals.append("信誉源未给出可用网络类型；保留其显式风险值，不以默认值覆盖。")
        if ip_type == "未知":
            ip_type = "网络类型未知"

    risk = max(0, min(100, risk))
    return _apply_ping0_native_constraint(
        IpClassification(
            ip_type=ip_type,
            risk_score=risk,
            ...
            limitations=[
                ...
                "Ping0 的非 IDC 标签包含商业、校园等网络，不能单独作为真实家宽证据。",
...
def _apply_ping0_native_constraint(
    classification: IpClassification,
    ping0: Optional[Ping0Quality],
) -> IpClassification:
    if not ping0 or not ping0.has_paid_quality or ping0.isnative is not False:
        return classification
    ip_type = classification.ip_type
    if "非原生" not in ip_type and "广播" not in ip_type:
        ip_type += "（非原生/广播）"
    signals = list(classification.signals)
    marker = "Ping0 isnative=False：非原生/广播 IP，自动选优降级为待复核。"
    if marker not in signals:
        signals.append(marker)
    risk = max(45, classification.risk_score)
...
def _classify_from_reputation(
    ...
    has_ping0_quality = bool(ping0 and ping0.has_paid_quality)
    if not ok_results and not has_ping0_quality:
        return None
    ...
    explicit_risk_floor = _explicit_reputation_risk_floor(ok_results, ping0)
    category_summary = _category_vote_summary(categories)
    if category_summary:
        signals.append("多源类型投票: " + category_summary)

    if ping0 and ping0.has_paid_quality and ping0.iprisk is not None and ping0.iprisk >= 70:
    ...
    if suspicious or ping0_suspicious:
        ...
        if has_anonymity_signal and residentialish:
            ...
            ip_type = "高风险出口可疑"
        suspicious_sources = {item.source for item in suspicious if item.source}
        confidence = "高" if len(suspicious_sources) >= 2 or risk >= 85 or ping0_suspicious else "中"
        return IpClassification(
            ip_type=ip_type,
            risk_score=risk,
            risk_label=_risk_label(risk),
    ...
    if hosting:
        ...
        risk = max(risk_floor, min(100, base_risk), explicit_risk_floor or 0)
        if residential:
            signals.append("多源冲突: 家宽信号与 IDC/机房信号同时存在，按更保守的机房风险处理。")
        return IpClassification(
            ip_type="家宽/机房冲突" if residential else "IDC/云机房",
            risk_score=risk,
            risk_label=_risk_label(risk),
            confidence="中" if residential else _category_vote_confidence(hosting),
            signals=signals,
            limitations=[
    ...
    if residential:
        ...
        if conflict_votes:
            ...
            risk = max(38, min(100, base_risk + 16), explicit_risk_floor or 0)
        ...
        risk = max(base_risk, explicit_risk_floor or 0)
        return IpClassification(
            ip_type="家庭宽带/住宅 IP",
            risk_score=max(0, risk),
            risk_label=_risk_label(max(0, risk)),
            confidence=_category_vote_confidence(residential),
            signals=signals,
            limitations=[
    ...
    if mobile:
        risk = max(
            max(_category_vote_risk(item, 24) for item in mobile),
            explicit_risk_floor or 0,
        )
        return IpClassification(
            ip_type="蜂窝/移动网络",
            risk_score=max(0, risk),
            risk_label=_risk_label(max(0, risk)),
            confidence=_category_vote_confidence(mobile),
            signals=signals,
            limitations=[
    ...
    if business:
        risk = max(
            max(_category_vote_risk(item, 34) for item in business),
            explicit_risk_floor or 0,
        )
        return IpClassification(
            ip_type="企业/商宽 IP",
            risk_score=max(0, risk),
            risk_label=_risk_label(max(0, risk)),
            confidence=_category_vote_confidence(business),
            signals=signals,
            limitations=[
...
def _explicit_reputation_risk_floor(
    results: list[ReputationInfo],
    ping0: Optional[Ping0Quality] = None,
) -> Optional[int]:
    values = [
        score
        for item in results
        if item.ok and (score := _explicit_reputation_score(item)) is not None
    ]
    if ping0 and ping0.has_paid_quality and ping0.iprisk is not None:
        values.append(ping0.iprisk)
    if ping0 and ping0.has_paid_quality and ping0.isnative is False:
        values.append(45)
    if not values:
        return None
...
def _reputation_score_is_suspicious(result: ReputationInfo) -> bool:
    score = _explicit_reputation_score(result)
    if score is None:
        return False
    # ProxyCheck's conservative guidance marks 76-100 as the deny range;
    # 51-75 is a challenge/review range and must not be promoted to a hard
    # high-risk verdict solely by crossing an arbitrary local threshold.
    threshold = 76 if result.source == "proxycheck" else 75
...
def _category_vote_confidence(votes: list[ReputationInfo | str]) -> str:
    ...
    sources = {
        vote.source
        for vote in votes
        if isinstance(vote, ReputationInfo) and vote.source
    }
    if len(sources) >= 2:
        return "高"
    return "中"
...
def _reputation_category_votes(
    ...
    for result in results:
        category = _reputation_network_category(result)
        if category in categories:
            categories[category].append(result)
        for flag_name, flag_category, allowed_sources in (
            ("residential", "residential", {SERVICE_NETCOFFEE}),
            ("mobile", "mobile", {SERVICE_NETCOFFEE, SERVICE_IPAPI}),
            ("hosting", "hosting", None),
        ):
            if (
                result.flags.get(flag_name)
                and category != flag_category
                and (allowed_sources is None or result.source in allowed_sources)
            ):
                categories[flag_category].append(result)
...
def reputation_has_network_classification(
    results: list[ReputationInfo],
    ping0: Optional[Ping0Quality] = None,
) -> bool:
    return any(_reputation_category_votes([item for item in results if item.ok], ping0).values())


def reputation_has_usable_evidence(result: ReputationInfo) -> bool:
    if not result.ok:
        return False
    return bool(
        result.network_type
        or _explicit_reputation_score(result) is not None
        or result.flags
        or result.shared_count is not None
        or result.subnet_shared_count is not None
    )


def _reputation_network_category(result: ReputationInfo) -> str:
    normalized = str(result.network_type or "").strip().lower().replace("_", " ").replace("-", " ")
    # ipapi.is documents company/asn type=isp as ownership metadata, not proof
    # that the specific address is a consumer residential assignment.
    if result.source == "ipapi" and normalized in {"isp", "fixed line isp"}:
        return ""
    return _network_type_category(result.network_type)
...
def _should_rotate_api_key(error: str) -> bool:
    text = str(error or "").strip().lower()
    if not text:
        return False
    if re.search(r"\b(?:401|403|429)\b", text):
        return True
    return any(
        marker in text
        for marker in (
...
def _http_result_error_text(result: HttpResult, fallback: str) -> str:
    ...
    if result.text:
        ...
        if isinstance(data, dict):
            payload = _payload_dict(data, "data", "result")
            detail = _payload_error_text(payload, _payload_error_text(data, result.error or fallback))
    detail = detail or result.error or fallback
    if result.status_code:
        status = f"HTTP {result.status_code}"
        if status.lower() not in detail.lower():
...
def _geo_text(data: dict[str, Any], *keys: str) -> str:
    value = _first_text(data, *keys)
    if value.casefold() in {"n/a", "na", "unknown", "null", "none", "-", "--"}:
        return ""
    return value


def _geo_country_code(data: dict[str, Any], *keys: str) -> str:
    return _geo_text(data, *keys).upper()
...
def _apply_netcoffee_payload(
    ...
    *,
    ...
    if not supplement:
        ...
        info.country_code = _geo_country_code(payload, "countryCode", "country_code", "isocode")
        info.country = _geo_text(payload, "country", "country_name")
        info.region = _geo_text(payload, "region", "state", "province")
        info.city = _geo_text(payload, "city")
    ...
    if not supplement:
        ...
        if verdict_label:
            ...
            if confidence is not None:
                info.confidence_score = max(info.confidence_score or 0, confidence)
...
def _netcoffee_network_type(payload: dict[str, Any]) -> str:
    ...
    if _optional_boolish(_first_value(payload, "isResidential", "is_residential", "residential")) is True:
        return "residential"
    for key in ("network_type", "type"):
        network_type = _first_text(payload, key)
        if network_type and _network_type_category(network_type):
            return network_type
    for key in ("company_type", "asn_kind"):
        network_type = _first_text(payload, key)
        category = _network_type_category(network_type)
...
def _netcoffee_flags(payload: dict[str, Any]) -> dict[str, bool]:
    abuser_risk = _netcoffee_abuser_risk(payload)
    flags = {
        ...
        "residential": _optional_boolish(
            _first_value(payload, "isResidential", "is_residential", "residential")
        ),
    }
    if abuser_risk is not None or any(
        key in payload
        for key in ("is_abuser", "abuser", "abuser_score", "abuser_level")
    ):
        flags["abuser"] = abuser_risk is not None
...
def _has_abuse_flag(flags: dict[str, bool]) -> bool:
    return any(
        flags.get(key, False)
        for key in (
            "scraper",
...
def _extract_proxycheck_record(data: dict[str, Any], ip: str) -> Optional[dict[str, Any]]:
    ...
    if isinstance(data.get("result"), dict):
        ...
        for key, value in result.items():
            if isinstance(value, dict) and _same_ip_address(str(key), ip):
                return value
        if "network" in result or "detections" in result:
            return result
...
def _same_ip_address(left: str, right: str) -> bool:
    try:
        return ipaddress.ip_address(str(left).strip().strip("[]")) == ipaddress.ip_address(
            str(right).strip().strip("[]")
        )
    except ValueError:
        return False
